...
function get_possible_prime_from_col(col::T, line::T) where {T<:Integer}
  return (12 * line) - prime_parents_subtractor[col]
end

function find_ceiling_n(col::T, possible_prime::T) where {T<:Integer}
  n = floor(((possible_prime - col) / 12) + 1)
  return T(n)
end

const skips = Set{Tuple{Int,Int}}()
const possible_primes = Set{Int}()
const confirmed_non_primes = Set{Int}()
...
for n in 1:100000
  possible_prime = get_possible_prime_from_col(5, n)
  push!(possible_primes, possible_prime)
  check_prime(possible_prime, n)
end
